...
function initializeMatterPlugin() {
  ...
  function addVisibleTargets() {
    console.log('addVisibleTargets 函式被呼叫'); // 偵錯日誌
    // 先找所有可見文字節點
  ...
  console.log('addVisibleTargets 函式執行完畢'); // 偵錯日誌
  addVisibleTargets();
  // window.removeEventListener('scroll', addVisibleTargets);
  window.removeEventListener('resize', addVisibleTargets);
  document.removeEventListener('DOMContentLoaded', addVisibleTargets);
  window.addEventListener('scroll', () => {
    console.log('scroll 事件被觸發'); // 偵錯日誌
    addVisibleTargets();
  });
  window.addEventListener('resize', () => {
    console.log('resize 事件被觸發'); // 偵錯日誌
    addVisibleTargets();
  });
  document.addEventListener('DOMContentLoaded', () => {
    console.log('DOMContentLoaded 事件被觸發'); // 偵錯日誌
    addVisibleTargets();
  });
  ...
  function shatterElement(target) {
    if (!target || !target.domElement) return;
    ...
    for (let i = 0; i < numParticles; i++) {
      ...
      particle.style.width = (rect.width / 10) + 'px';
      particle.style.height = (rect.height / 10) + 'px';
      particle.style.background = el.style.background || '#888'; // Use element's background or default
      ...
      // Apply velocity and fade out
      const lifespan = 1000; // milliseconds
      particle.animate([
        { transform: `translate(0, 0)`, opacity: 1 },
        { transform: `translate(${vx * lifespan / 1000}px, ${vy * lifespan / 1000}px)`, opacity: 0 }
      ], {
        duration: lifespan,
        easing: 'ease-out',
        fill: 'forwards'
  ...
  // 動畫渲染（射擊體只做位置同步）
  function renderLoop() {
    console.log('renderLoop 函式正在執行'); // 偵錯日誌
    ...
    Composite.allBodies(world).forEach(body => {
      ...
      console.log(`renderLoop: window.scrollY = ${window.scrollY}`); // 偵錯日誌
      const scrollX = window.scrollX;
      const scrollY = window.scrollY;
      const fixedX = pos.x - scrollX - (el.offsetWidth / 2);
      const fixedY = pos.y - scrollY - (el.offsetHeight / 2);
  ...
  Events.on(engine, 'afterUpdate', function() {
      ...
      Matter.Composite.allBodies(engine.world).forEach(body => {
          if ((body.label === 'fallingChar' || body.label === 'bombChar') && body.position.y > window.innerHeight + 100) { // 添加緩衝區
      ...
      if (shooterElement && shooterElement.parentElement) {
        ...
        console.log(`afterUpdate 同步 shooterElement 位置，shooterElement 存在: ${!!shooterElement}，計算出的 left: ${calculatedLeft}, top: ${calculatedTop}, shooterX: ${shooterX}, shooterY: ${shooterY}`); // 偵錯日誌
      }
